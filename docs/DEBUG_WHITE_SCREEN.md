...
- 診断パネルは **Debug のときだけ** 画面に出るコードになっています。**Release のまま Run しているとパネルは出ません。**
...
1. 画面 **最上部** に **赤い横帯**（「YoiYoi DEBUG …」の文言）が出ます。これが見えれば **SwiftUI の overlay まで描画できている** 証拠です。
2. **赤帯が出ない**のに真っ白だけのときは、**Release で Run していないか**、**別ターゲット**を実行していないかを確認する。

#### パネルが「出た」とき

- アプリの SwiftUI は **少なくともウィンドウに描画できている** 状態です。
- そのときは、パネル内の **最後の行まで**（またはできるだけ多く）をコピーして、共有すると原因の切り分けがしやすくなります。

#### パネルが「出ない」のに画面が真っ白だけのとき
...
**Debug** 構成で Run してください（Release では画面左上の診断パネルは出ませんが、`os_log` は残ります）。  
※ 上の「初心者向け」に、Scheme の開き方から書いています。

## 2. 画面上の「起動診断」パネルを見る

画面上部に **赤い DEBUG 帯**（`DebugLaunchOverlay`）が出れば、**SwiftUI は少なくとも `AppRootView` まで描画できています**。  
真っ白で **赤い DEBUG 帯も見えない** 場合は次のどちらかが多いです。

- **Release で Run** している（赤帯は DEBUG のみ）  
...
## Clean → Run で「赤い DEBUG 帯」を確認する（詳しい手順）

**DEBUG ビルド**のときだけ次が出ます。

| 名前 | 画面上の見た目 | 意味 |
|------|----------------|------|
| **赤い DEBUG 帯** | 画面上部の **赤い横長の帯**に白文字（「YoiYoi DEBUG …」など） | **`AppRootView` の `overlay`**（`DebugLaunchOverlay`）が描画されている。 |
...
### 手順 3 — シミュレーター画面で何を見るか

起動後、**すぐ**次を確認します。

1. **赤い DEBUG 帯**  
   - **見える** → **SwiftUI の overlay まで描画パイプラインに乗っている**  
   - **見えない** → **Release** か、オーバーレイが 0 サイズのまま等の可能性  

2. **その下にホームのグラデーションやタブ**が出るか  
