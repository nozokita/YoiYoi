...
> 最終更新: 2026.05.31  
> 方針: v1.0 はアプリ独自の利用イベント記録、外部分析 SDK、広告 SDK、アカウント、クラウド同期を入れず、飲酒記録と設定は端末内だけに保存する。公開初期の利用傾向は App Store Connect の標準指標だけで確認する。
...
- 現行実装では、飲酒記録、設定、AIコメント用の集計値を端末外へ送信しない。
- Apple の App Privacy Details では「collect」は、開発者または第三者がアクセスできる形で端末外へ送信されることを指すため、現行版は **Data Not Collected** を前提に回答する。
- 将来、広告 SDK、外部分析、クラッシュレポート、リモート設定、クラウド同期を入れる場合は、リリース前に Privacy Policy URL と App Privacy Details を更新する。
...
- App Store Connect のクラッシュ、レビュー、インストール推移、セッション、継続指標を確認する。
- アプリ独自の利用イベント記録は入れないため、細かな機能別利用率は取得しない。公開初期は Apple 公式指標とユーザーフィードバックを優先する。
