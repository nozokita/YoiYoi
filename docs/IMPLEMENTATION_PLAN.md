# YoiYoi Lean MVP 実装計画 v2.0
> 最終更新: 2026.06.01
>
> 仕様は [SPEC.md](./SPEC.md)、画面方針は [DESIGN.md](./DESIGN.md) を正とする。

## 目標

楽しく、無理しないペースでアルコール摂取量をコントロールできる、飲酒記録は端末内保存の iPhone アプリへ整理する。MVP は日本語と英語を提供し、言語追加に耐える構造を維持する。Phase 3 では任意同意の匿名 analytics を追加し、DAU/MAU と継続率を把握できるようにする。

## 実装原則

- 飲酒記録と設定は SwiftData で端末内に保持する。
- ローカル通知以外の外部通信を初期公開の前提にしない。Phase 3 では、明示的に同意したユーザーのみ匿名・粗粒度の analytics を送信する。
- UI 文言は JA/EN を同時に追加し、`SupportedLanguage` を通して切り替える。
- 「設定した目安まであと28g」のように、ユーザーの設定値との比較として表現する。
- プライバシーポリシーはアプリ内と公開ページの双方から到達できる状態を維持する。
- 実用機能と安全寄りのAI支援は無料で維持し、課金は口調・キャラ・表情・テーマ・演出などの表現価値に寄せる。

## 移行フェーズ

### Phase 1: 旧機能整理

状態: 反映済み。

| 作業 | 完了条件 |
|------|----------|
| クラウド連携と共有画面のコードを削除 | アプリターゲットに不要な依存・参照がない |
| 匿名表示名と初回同意画面を削除 | 初回導線が言語選択から開始する |
| CSV 出力と旧法務リンクを削除 | 設定にプライバシーポリシーだけが残る |
| 公開文書を更新 | 端末内保存と JA/EN 方針が明記される |

維持するもの:

- `YoiYoiAppDelegate` のフォアグラウンド通知表示処理。
- `SupportedLanguage`、言語選択画面、設定画面の言語切替。
- 既存の飲酒記録、カレンダー、目安設定、ローカル通知。

### Phase 2: クイック記録

状態: 反映済み。

| 作業 | 実装内容 |
|------|----------|
| `QuickDrinkPreset` を追加 | よく飲むドリンクを最大 6 件保存 |
| 最近の記録を算出 | 種類・容量・度数・杯数で重複排除し最大 5 件表示 |
| ホームに `すぐ記録` を追加 | タップで記録し 5 秒間の取り消しを提供 |
| 記録シートと連携 | ショートカットから値を引き継いで編集可能にする |

テスト:

- お気に入り上限と並べ替え。
- 最近の記録の重複排除と順序。
- ワンタップ保存と取り消し。
- JA/EN 表示の切替後にも保存値が変わらないこと。

### Phase 3: 飲み会セッションと通知

状態: 反映済み。

| 作業 | 実装内容 |
|------|----------|
| `DrinkingSession` を追加 | 開始・終了・現在のセッションを管理 |
| 記録を紐付け | セッション中の新規記録へ `sessionID` を設定 |
| 水分補給リマインド | 選択した間隔でローカル通知を予約 |
| セッション終了 | 未送信の通知を取り消し、振り返りを表示 |

テスト:

- セッション開始・終了と記録の紐付け。
- 通知の予約・取り消し。
- 通知権限がない場合でもセッション操作が継続できること。

### Phase 4: ローカルコーチ

状態: 反映済み。

| 経路 | 条件 | 実装 |
|------|------|------|
| 定型メッセージ | 全対応 OS | 必須のフォールバック |
| Foundation Models | iOS 26+ かつ利用可能 | 端末上で短い声かけを生成 |

実装事項:

- コーチ性格選択をオンボーディングと設定に追加する。
- 生成不可または失敗はエラー画面にせず定型メッセージへ切り替える。
- 診断や飲酒推奨に見える出力を行わない安全制約を共通化する。

### Phase 5: リリース準備

| 作業 | 確認内容 |
|------|----------|
| プライバシー表示 | 初回画面、設定、App Store 表示で端末内保存を JA/EN で明記 |
| 画面 QA | 日英、Dynamic Type、小型 iPhone で主要操作が利用可能 |
| データ移行 | 既存記録と目安設定を保持し、新規フィールドを既定値で補完 |
| 安全表現 | 目安を安全量または追加摂取可能量として扱わない |

### Phase 6: Phase 3 Analytics and Growth

状態: 仕様作成済み。実装前。

詳細は [PHASE3_ANALYTICS_AND_GROWTH.md](./PHASE3_ANALYTICS_AND_GROWTH.md) を正とする。

| 作業 | 完了条件 |
|------|----------|
| Privacy/Store 方針更新 | App Store 文言、プライバシーポリシー、App Privacy Details 回答案が一致する |
| Consent UI | 設定から匿名 analytics の ON/OFF と分析IDリセットができる |
| Event Taxonomy 整理 | 送信イベントと属性が許可リストに限定される |
| Analytics Client | opt-in の場合のみバッチ送信し、OFFでは通信しない |
| Self-hosted API | `POST /v1/events`、スキーマ検証、レート制限、保持期間、集計ジョブを実装 |
| Dashboard | DAU/MAU、Retention、Onboarding、Logging、AI、Unlock、Paywall funnel を確認できる |
| Unlocks | 累積カウントアップ、口調/キャラ/テーマ解禁、買い切り解放を実装 |
| Character/Copy | `ドクター` を廃止し、`アナリスト` / `データさん` を追加 |

Phase 6 の公開ゲート:

- analytics OFF で外部送信が発生しない。
- 飲酒記録の生データ、正確な飲酒日時、AIコメント全文、自由入力、位置情報、IDFA が送信されない。
- App Privacy Details と Privacy Policy が実装と一致している。
- 実用機能と安全寄りのAI支援が課金でロックされていない。

## 現行画面構成

| 画面 | 現在の責務 |
|------|------------|----------|
| 初回画面 | 言語選択、性別・目安、端末内保存の訴求、コーチ性格選択 |
| ホーム | 今日の量、目安との差、クイック記録、コーチ、飲み会モード |
| カレンダー | 日々の記録振り返り、ラストオーダー達成表示 |
| 設定 | 目安、通知、言語、プライバシー、免責、コーチ設定 |
| 記録シート | ドリンク入力、お気に入り登録、最近の内容再利用、保存 |
| プライバシー/利用状況データ | Phase 3 で追加。匿名 analytics のON/OFF、分析IDリセット |

## 検証コマンド

実装単位ごとに次を行う。

```sh
git diff --check
xcodebuild -list -project YoiYoi/YoiYoi.xcodeproj
xcodebuild test -project YoiYoi/YoiYoi.xcodeproj -scheme YoiYoi \
  -destination 'platform=iOS Simulator,name=iPhone 16 Pro'
```

シミュレーター環境に指定端末がない場合は、利用可能な iPhone シミュレーターを選んで同じテストを実行する。

## 完了判定

- 日本語・英語の切替が残り、初回画面と設定から操作できる。
- 飲酒記録・目安設定が端末内で保存される。
- 不要になった旧機能への UI 導線、依存、テスト、公開用説明が残っていない。
- ホームの差分表現が「設定した目安まであと28g」に統一されている。
- ビルドと対象テストが通過し、公開文書が実装状態を正しく示している。
